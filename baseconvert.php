function baseconverter ( $value, $charset='0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+-', $from_charset = '0123456789abcdef', $pad = true  )
// Author: Ryan Griggs
// Copyright 2020 - MIT license
// Convert positive integer values between bases.  By default, convert from Base-16 to Base-N
...
// $from_charset = the charset for the base-N to convert $value from.  Default = base-16
...
// Does not support negative numbers or floating point
{
	...
	if ( ! is_array( $charset ) ) {
		$charset = mb_str_split( $charset );
	}	// $charset contains array of characters

	// Convert $from_charset to array
	if ( ! is_array( $from_charset ) ) {
		$from_charset = mb_str_split( $from_charset );
	}
	// Get base sizes:
	$from_base = count( $from_charset );	// Converting from this base
	if ( $from_base < 2 ) {
		throw new Exception( "Invalid from_charset. Must be > 1 character." );
	}	// Base-1 is invalid.
	$to_base = count( $charset );			// Converting to this base
	if ( $to_base < 2 ) {
		throw new Exception( "Invalid charset. Must be > 1 character." );
	}	// Base-1 is invalid.
	...
	foreach ( $value as $i => $char ) {
		...
		if ( $ordinal === false ) {
			throw new Exception( "Invalid ordinal in baseConvert value: $char" );
		}

		// Append value of this character to the result
		// $intval += ( $from_base ** $magnitude ) * $ordinal;\
        $intval = bcadd( $intval, bcmul( bcpow( $from_base, $magnitude), $ordinal)) ;
	}
